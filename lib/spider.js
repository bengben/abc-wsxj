...
var spider = {
  ...
  async getInfoById() {
    var url = "https://xj.ccgp-chongqing.gov.cn/enquiry/notice!enquiryNotice.action?notic_seq=880523582290268160";
    superagent.get(url).end(async (err, res) => {
      ...
      info.buydo = {
        org_remark: $('.notice-contact').eq(0).find('.contact-type-name').text(),
        org_name: $('.contact-detail').find('li').eq(0).find('.contact-content').text(),
        org_person: $('.contact-detail').find('li').eq(1).find('.contact-content').text(),
        org_phone: $('.contact-detail').find('li').eq(2).find('.contact-content').text()
      }; //采购执行方
      info.buyreq = {
        org_remark: $('.notice-contact.ng-scope').find('.contact-type-name').text(),
        org_name: $('.notice-contact.ng-scope').find('li').eq(0).find('.contact-content').text(),
        org_person: $('.notice-contact.ng-scope').find('li').eq(1).find('.contact-content').text(),
        org_phone: $('.notice-contact.ng-scope').find('li').eq(2).find('.contact-content').text()
      };; //采购需求方
      // var attch =
      console.log('单位：', info);
      // return info;
      var att_url = 'https://www.ccgp-chongqing.gov.cn/enquiry/notice!getAttachmentsByNotice_seq.action?notic_seq=880523582290268160';
      try {
